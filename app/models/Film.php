...
public function reserveFilm($data){
      $this->db->query(' INSERT INTO reservation ( salle , reservation_date , n_place ) VALUES( :salle , :reservation_date  , :n_place ) ');
      // Bind values
      $this->db->bind(':salle', $data['salle']);
      $this->db->bind(':reservation_date', $data['reservation_date']);
      $this->db->bind(':n_place', $data['n_place']);
      if($this->db->execute()){
        return true;
      } else {
        return false;
